...
/**
 * A reentrant mutual exclusion {@link Lock} with the same basic behavior and semantics as the
 * implicit monitor lock accessed using {@code synchronized} methods and statements, but with
 * extended capabilities.
 *
 * <p>A {@code ReentrantLock} is <em>owned</em> by the thread last
 * successfully locking, but not yet unlocking it. A thread invoking {@code lock} will return,
 * successfully acquiring the lock, when the lock is not owned by another thread. The method will
 * return immediately if the current thread already owns the lock. This can be checked using methods
 ...
 * not guarantee any particular access order.  Programs using fair locks accessed by many threads
 * may display lower overall throughput (i.e., are slower; often much slower) than those using the
 ...
 * Also note that the untimed {@link #tryLock()} method does not honor the fairness setting. It will
 * succeed if the lock is available even if other threads are waiting.
 *
 * <p>It is recommended practice to <em>always</em> immediately
 * follow a call to {@code lock} with a {@code try} block, most typically in a before/after
 ...
 * class defines a number of {@code public} and {@code protected} methods for inspecting the state
 * of the lock.  Some of these methods are only useful for instrumentation and monitoring.
 *
 * <p>Serialization of this class behaves in the same way as built-in
 * locks: a deserialized lock is in the unlocked state, regardless of its state when serialized.
 *
 * <p>This lock supports a maximum of 2147483647 recursive locks by
 * the same thread. Attempts to exceed this limit result in {@link Error} throws from locking
 * methods.
 *
 * @author Doug Lea
 * @since 1.5
...
public class ReentrantLock implements Lock, java.io.Serializable {
  ...
  abstract static class Sync extends AbstractQueuedSynchronizer {
    ...
    /**
     * Performs {@link Lock#lock}. The main reason for subclassing is to allow fast path for nonfair
     * version.
     */
    abstract void lock();
    ...
    final boolean nonfairTryAcquire(int acquires) {
      ...
      int c = getState();  // volatile 类型的数据 用state 表示当前线程是否抢到了锁
      if (c == 0) {
        // 0 表示没有抢到锁，则使用cas的方式 尝试去进行锁的竞争
  ...
  static final class FairSync extends Sync {
    ...
    /**
     * Fair version of tryAcquire.  Don't grant access unless recursive call or no waiters or is
     * first.
     */
    protected final boolean tryAcquire(int acquires) {
      final Thread current = Thread.currentThread();
  ...
  /**
   * Creates an instance of {@code ReentrantLock}. This is equivalent to using {@code
   * ReentrantLock(false)}.
   */
   ...
   * current thread becomes disabled for thread scheduling purposes and lies dormant until the lock
   ...
   * returns immediately with the value {@code true}, setting the lock hold count to one. Even when
   * this lock has been set to use a fair ordering policy, a call to {@code tryLock()} <em>will</em>
   * immediately acquire the lock if it is available, whether or not other threads are currently
   * waiting for the lock. This &quot;barging&quot; behavior can be useful in certain circumstances,
   * even though it breaks fairness. If you want to honor the fairness setting for this lock, then
   * use {@link #tryLock(long, TimeUnit) tryLock(0, TimeUnit.SECONDS) } which is almost equivalent
   ...
   *
   * <p>Acquires the lock if it is not held by another thread and returns
   * immediately with the value {@code true}, setting the lock hold count to one. If this lock has
   * been set to use a fair ordering policy then an available lock <em>will not</em> be acquired if
   * any other threads are waiting for the lock. This is in contrast to the {@link #tryLock()}
   * method. If you want a timed {@code tryLock} that does permit barging on a fair lock then
   ...
   *
   * <p>If the current thread
   * already holds this lock then the hold count is incremented by one and the method returns {@code
   * true}.
   *
   * <p>If the lock is held by another thread then the
   * current thread becomes disabled for thread scheduling purposes and lies dormant until one of
   ...
   * @return {@code true} if the lock was free and was acquired by the current thread, or the lock
   * the lock was already held by the current thread; and {@code false} if the waiting time elapsed
   ...
   * count is decremented.  If the hold count is now zero then the lock is released.  If the current
   ...
   * usages as do the {@link Object} monitor methods ({@link Object#wait() wait}, {@link
   * Object#notify notify}, and {@link Object#notifyAll notifyAll}) when used with the built-in
   * monitor lock.
   *
   * <ul>
   *
   ...
   * debugging purposes. For example, if a certain section of code should not be entered with the
   * lock already held then we can assert that fact:
   *
   * <pre> {@code
   * class X {
   ...
   * built-in monitor locks, this method is typically used for debugging and testing. For example, a
   * method that should only be called while a lock is held can assert that this is the case:
   *
   * <pre> {@code
   * class X {
   *   ReentrantLock lock = new ReentrantLock();
   ...
   * acquire this lock.  This method is designed primarily for use in monitoring of the system
   * state.
   *
  ...
  /**
   * Returns an estimate of the number of threads waiting to acquire this lock.  The value is only
   ...
   * particular order.  This method is designed to facilitate construction of subclasses that
