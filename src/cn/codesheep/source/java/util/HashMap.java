 ...
 * operations (<tt>get</tt> and <tt>put</tt>), assuming the hash function disperses the elements
 * properly among the buckets.  Iteration over collection views requires time proportional to the
 * "capacity" of the
 * <tt>HashMap</tt> instance (the number of buckets) plus its size (the number
 * of key-value mappings).  Thus, it's very important not to set the initial capacity too high (or
 ...
 * get before its capacity is automatically increased.  When the number of entries in the hash table
 * exceeds the product of the load factor and the current capacity, the hash table is
 * <i>rehashed</i> (that is, internal data structures are rebuilt) so that the hash table has
 * approximately twice the number of buckets.
 *
 * <p>As a general rule, the default load factor (.75) offers a good
 * tradeoff between time and space costs.  Higher values decrease the space overhead but increase
 * the lookup cost (reflected in most of the operations of the <tt>HashMap</tt> class, including
 * <tt>get</tt> and <tt>put</tt>).  The expected number of entries in
 * the map and its load factor should be taken into account when setting its initial capacity, so as
 * to minimize the number of rehash operations.  If the initial capacity is greater than the maximum
 * number of entries divided by the load factor, no rehash operations will ever occur.
 *
 * <p>If many mappings are to be stored in a <tt>HashMap</tt>
 * instance, creating it with a sufficiently large capacity will allow the mappings to be stored
 * more efficiently than letting it perform automatic rehashing as needed to grow the table.  Note
 * that using many keys with the same {@code hashCode()} is a sure way to slow down performance of
 * any hash table. To ameliorate impact, when keys are {@link Comparable}, this class may use
 * comparison order among keys to help break ties.
 *
 * <p><strong>Note that this implementation is not synchronized.</strong>
 * If multiple threads access a hash map concurrently, and at least one of the threads modifies the
 * map structurally, it <i>must</i> be synchronized externally.  (A structural modification is any
 * operation that adds or deletes one or more mappings; merely changing the value associated with a
 * key that an instance already contains is not a structural modification.)  This is typically
 ...
 * time, to prevent accidental
 * unsynchronized access to the map:<pre>
 *   Map m = Collections.synchronizedMap(new HashMap(...));</pre>
 *
 * <p>The iterators returned by all of this class's "collection view methods"
 * are <i>fail-fast</i>: if the map is structurally modified at any time after the iterator is
 ...
 * as it is, generally speaking, impossible to make any hard guarantees in the presence of
 ...
 * @see TreeMap
 * @see Hashtable
 * @since 1.2 hashmap其实是在数组的基础上实现的，一个加强版的数组
 */
public class HashMap<K, V> extends AbstractMap<K, V>
  ...
  /**
   * The load factor used when none specified in constructor. capacity / size > loadFactor
   * 也就是达到loadFactor比例时，就需要扩容，
   */
  ...
  /**
   * 底层存储的数组 The table, initialized on first use, and resized as necessary. When allocated, length
   * is always a power of two. (We also tolerate length zero in some operations to allow
   * bootstrapping mechanics that are currently not needed.) 被 transient 修饰的成员变量，不会被序列化
   * 也就是在类的实例化对象过程中，会被忽略
   */
  ...
  /**
   * The number of key-value mappings contained in this map. key-value 的键值对的数量
   */
  transient int size;

  /**
   * hashmap被修改的的次数 The number of times this HashMap has been structurally modified Structural
   * modifications are those that change the number of mappings in the HashMap or otherwise modify
   * its internal structure (e.g., rehash).  This field is used to make iterators on
   ...
   * load factor (0.75).
   * <p>
   * 创建一个初始化容量为16的hashmap对象 在该构造器上我们并没有看到table数组的初始化，它是延迟初始化，在我们开始往hashmap中添加key-value键值对时 resize()
   * 方法中中才开始真正的初始化
   */
  public HashMap() {
   ...
   * explicitly maps the key to {@code null}. The {@link #containsKey containsKey} operation may be
  ...
  final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
    ...
    // i 对应table的位置
    int n, i;
    // <1> 如果 table 未初始化，或者容量为0， 则进行扩容
    if ((tab = table) == null || (n = tab.length) == 0)
    // 扩容
    {
      n = (tab = resize()).length;
    }
    // <2> 计算索引的位置   (n - 1) & hash; 如果为空直接创建node
    ...
    // <3> 如果对应位置的 Node 节点非空，则可能存在哈希冲突
      ...
      Node<K, V> e;
      K k;
      //  找到的p节点 就是要找的，则则直接使用即可
      if (p.hash == hash &&   // 判断hash值是否相等
          ((k = p.key) == key || (key != null && key.equals(k))))   // 判断key是否相等
      {
        e = p;
      } else if (p instanceof TreeNode)   // 如果找到的p节点 是红黑树的node节点，则直接添加到树中
      {
        ...
        for (int binCount = 0; ; ++binCount) {
          if ((e = p.next) == null) {
            // 说明已经遍历到链表的尾巴，则说明key 在hashmap中不存在，则需要创建node节点
      ...
      if (e != null) { // existing mapping for key
        ...
        afterNodeAccess(e);   // linkedHashMap 需要做的扩展逻辑
        return oldValue;
      }
    }
    ++modCount;
  ...
  final Node<K, V>[] resize() {
    ...
    Node<K, V>[] newTab = (Node<K, V>[]) new Node[newCap]; // 创建xin的node数组， 赋值给table属性
    table = newTab;
    // 如果老的table 数组非空，则需要进行一波搬运
    if (oldTab != null) {
      //循环遍历老的数组的容量
      for (int j = 0; j < oldCap; ++j) {
        Node<K, V> e;
        if ((e = oldTab[j]) != null) {   // 获得老的table数组第j位置的Node节点 e， 该索引位置
          // 置空老的table数组第j索引位置
          oldTab[j] = null;
          //  <2.1> 如果 e 节点只有一个元素，直接赋值给新的 table 即可
          if (e.next == null) {
            newTab[e.hash & (newCap - 1)] = e;
            // 如果e节点是红黑树节点，则通过红黑树分裂处理
          } else if (e instanceof TreeNode) {
            ((TreeNode<K, V>) e).split(this, newTab, j, oldCap);
          } else { // preserve order
            // 如果e节点是链表
            // loHead 代表链表中的第一个节点  loTail 代表链表中最后一个节点
            Node<K, V> loHead = null, loTail = null;
            // hiHead 需要重新定位的第一个node hiTail 代表需要重新定位的最后一个node
  ...
  final void treeifyBin(Node<K, V>[] tab, int hash) {
    int n, index;
    Node<K, V> e;
    // table 容量 还没有达到 能够树化的最小阈值， 则table 数组需要先扩容
  ...
  final Node<K, V> removeNode(int hash, Object key, Object value,
    ...
    Node<K, V> p; // hash 对应table 位置的p节点
    int n, index;
    // 《1》 查找hash 对应table 位置的p节点
    if ((tab = table) != null && (n = tab.length) > 0 &&
   ...
   * mappings), followed by the key (Object) and value (Object) for each key-value mapping.  The
